...
var (
	...
	migDir           = filepath.Join(GetProjectRootDir(), "/database/migrations")
	fixFile          = filepath.Join(GetProjectRootDir(), "/backend/internal/test/fixtures.go")
	selfFile         = filepath.Join(GetProjectRootDir(), "/backend/internal/test/test_database.go")
	defaultPoolPaths = []string{migDir, fixFile, selfFile}
)

func init() {
	// loads values from .env into the system
	if err := godotenv.Load(GetProjectRootDir() + "/backend/test.env"); err != nil {
		log.Print("test.env not found")
	}
...
func storePoolHash(t *testing.T, poolID string, hashPaths []string, fragments ...string) string {
	...
	if err != nil {
		t.Fatalf("Failed to create template hash for %v: %#v", poolID, err.Error())
	}
...
func ApplyMigrations(t *testing.T, db *sql.DB) (countMigrations int, err error) {
	...
	m := Migrate{
		DB:                     db,
		MigrationFilesGlobPath: migDir + "/*.sql",
	}
	countMigrations, err = m.Run()
