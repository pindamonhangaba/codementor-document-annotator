...
type LoginForm struct {
	...
	ApplID     string `json:"applID" binding:"required" example:"api"`
...
func (u *Authenticator) EmailLogin(log *LoginForm) (a *service.AuthResponse, err error) {
	...
	a = &service.AuthResponse{User: usr.User, Jwt: jwt}
	return a, err
}
...
func (p *PwdRecoverer) Run(email string, applID string, emailModel *string) (*service.ActionVerification, error) {
	...
	confirmationURL := p.Config.AuthURL + "/password_reset/" + ac.AcveID.String() + "/" + secret
...
func fromEmail(db service.DB, email, appldID string) (usr *UserAuthentication, err error) {
	...
	query := psql.Select(
		...
		"COALESCE(ae.email, ao.email) as email", "ae.password",
	).
		From(`"user" u`).
